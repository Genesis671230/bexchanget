...
<head>
	...
	<title>Trading courses | Partner Panel</title>
	<meta charset="utf-8">
					...
					<div id="kt_app_sidebar_menu_wrapper" class="app-sidebar-wrapper hover-scroll-overlay-y my-5" data-kt-scroll="true" data-kt-scroll-activate="true" data-kt-scroll-height="auto" data-kt-scroll-dependencies="#kt_app_sidebar_logo, #kt_app_sidebar_footer" data-kt-scroll-wrappers="#kt_app_sidebar_menu" data-kt-scroll-offset="5px" data-kt-scroll-save-state="true">
						...
						<div class="menu menu-column menu-rounded menu-sub-indention px-3" id="#kt_app_sidebar_menu" data-kt-menu="true" data-kt-menu-expand="false">
							...
							<div class="menu-item">
								...
								%>
								<!--begin:Menu link-->
								<a class="menu-link active" href="/panel/trading-course">
                                 <span class="menu-icon">
                                    <!--begin::Svg Icon | path: icons/duotune/general/gen002.svg-->
                                    <span class="svg-icon svg-icon-2">
                                       <svg width="24" height="24" viewbox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
							...
							<div class="col-xl-12 mb-xl-12">
								...
								<div class="card card-flush h-xl-100">
									...
									<div class="card-body pt-4 px-0">
										...
										<ul class="nav nav-pills nav-pills-custom item position-relative mx-9 mb-9" role="tablist">
											...
											<li class="nav-item col-3 mx-5 px-0" role="presentation">
												...
												<a class="nav-link d-flex justify-content-center w-100 border-0 h-100" data-bs-toggle="pill" href="#kt_list_widget_16_tab_2" aria-selected="false" role="tab" tabindex="-1">
													<!--begin::Subtitle-->
													<span class="nav-text text-gray-800 fw-bold fs-6 mb-3">Stable PUMP/DUMP</span>
													<!--end::Subtitle-->
													<!--begin::Bullet-->
													<span class="bullet-custom position-absolute z-index-2 bottom-0 w-100 h-4px bg-primary rounded"></span>
													<!--end::Bullet-->
										...
										<!--end::Nav-->
										<!--begin::Tab Content-->
										<div class="tab-content px-9 hover-scroll-overlay-y pe-7 me-3 mb-2" style="height: 900px">
											<!--begin::Tap pane-->
																...
																<select id="course_pairs" name="currnecy" aria-label="Select a Timezone" data-control="select2" data-placeholder="Select currency" class="form-select form-select-solid">
																	<%
																		...
																		for (Map<String, Object> coin : coins) {
																			...
																			out.println("<option value=\"" + symbol + "\">" + symbol + "_USDT</option>");
																		}
																	%>
																</select>
																<!--end::Select-->
														...
														<div class="card-body p-12">
															<div class="table-responsive">
																...
																<table class="table align-middle gs-0 gy-3 table-row-dashed table-striped">
																	...
																	<!--end::Table head-->
																	<!--begin::Table body-->
																	<tbody id="course_edited_stable_table">
																	<%
...
<script>
	...
	$("#save_edit_course").on("click", function() {
		...
		if(course_amount == '') {
			...
			$.ajax({
				...
				type: "POST",
				data: {
					action: "FAST_PUMP_EDIT",
					symbol: course_pairs,
					percent: course_amount
				},
				success: function(response) {
					...
					if(response == "1") {
						...
						$("#course_edited_table").load("../api/pumps?action=GET_FAST_PUMPS");
					} else if(response == "3") {
						noti("Произошла неизвестная ошибка, обратитесь в саппорт :(", "error");
					} else if(response == "5") {
						noti("Подождите, пока график достигнет предыдущих свечей", "error");
					}
				}
			})
		}
	...
	function deleteTradingPairsCourse(symbol) {
		noti("Процесс начался..", "info");
		$.ajax({
			url: "../api/pumps",
			type: "POST",
			data: {
				action: "FAST_PUMP_RESET",
				symbol: symbol
			},
			success: function(response) {
				console.log(response);
				if(response == "1") {
					...
					noti("Пампы/Дампы для данной пары не установлены", "error");
				} else if(response == "3") {
					noti("Произошла неизвестная ошибка, обратитесь в саппорт :(", "error");
				}
			}
			...
			$.ajax({
				...
				success: function(response) {
					if(response == "1") {
						...
						$("#course_edited_stable_table").load("../api/pumps?action=GET_STABLE_PUMPS");
						$("#course_edited_table").load("../api/pumps?action=GET_FAST_PUMPS");
					} else if(response == "2") {
	...
	function deleteTradingStablePairsCourse(symbol) {
		noti("Процесс начался..", "info");
		$.ajax({
			url: "../api/pumps",
			type: "POST",
			data: {
				action: "STABLE_PUMP_DELETE",
				symbol: symbol
			},
			success: function(response) {
				console.log(response);
