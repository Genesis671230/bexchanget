...
<%@ page import="ru.tiedie.cryptoexchange.data.DataManager" %>
<!DOCTYPE html>
<html lang="en">
<!--begin::Head-->
<head>
	<!-- Removed by WebCopy --><!--<base href="">--><!-- Removed by WebCopy -->
	<title>Domains | Partner Panel</title>
	<meta charset="utf-8">
				...
				<div class="app-sidebar-menu overflow-hidden flex-column-fluid">
					...
					<div id="kt_app_sidebar_menu_wrapper" class="app-sidebar-wrapper hover-scroll-overlay-y my-5" data-kt-scroll="true" data-kt-scroll-activate="true" data-kt-scroll-height="auto" data-kt-scroll-dependencies="#kt_app_sidebar_logo, #kt_app_sidebar_footer" data-kt-scroll-wrappers="#kt_app_sidebar_menu" data-kt-scroll-offset="5px" data-kt-scroll-save-state="true">
						...
						<div class="menu menu-column menu-rounded menu-sub-indention px-3" id="#kt_app_sidebar_menu" data-kt-menu="true" data-kt-menu-expand="false">
							...
							<div class="menu-item">
								<!--begin:Menu link-->
								<a class="menu-link active" href="/panel/domains">
                                 <span class="menu-icon">
                                    <!--begin::Svg Icon | path: icons/duotune/general/gen002.svg-->
                                    <span class="svg-icon svg-icon-2">
                                       <svg width="24" height="24" viewbox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
								...
								<div class="card-body py-0">
									...
									<div class="table-responsive">
										...
										<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_customers_logs">
											...
											<tbody>
											<%
												Map<String, String> domains = (Map<String, String>) request.getAttribute("domains");
												for (Map.Entry<String, String> entry : domains.entrySet()) {
													Object name = entry.getKey();
													Object icon = entry.getValue();
													out.println("<tr>\n" +
															"   <td class=\"min-w-70px text-dark fs-6\">\n" +
															"      <img style=\"width: 20px; margin-bottom: 0px; margin-right: 6px; background: #1f1d2c;\" src=\"" + icon + "\">" + name + "\n" +
															"   </td>\n" +
															"   <td><span class=\"label label-success\" style=\"background: #00d17a; padding: 4px; color: white; border-radius: 5px;\">Active</span></td>\n" +
															"   <td>\n" +
															"      <a data-bs-toggle=\"modal\" data-bs-target=\"#kt_edit_my_domain\" onclick=\"editMyDomain('" + name + "')\" class=\"btn btn-xs btn-primary edit-domain\" data-original-title=\"Edit settings\"><i class=\"fas fa-pencil-alt\"></i> Edit</a>\n" +
															"   </td>\n" +
															"</tr>");
												}
											%>
						...
						<div class="modal-body scroll-y px-10 px-lg-15 pt-0 pb-15">
							...
							<form id="kt_modal_new_target_form" class="form fv-plugins-bootstrap5 fv-plugins-framework" action="#">
								...
								<div class="d-flex flex-column mb-8 fv-row fv-plugins-icon-container">
									<!--begin::Label-->
									<label class="d-flex align-items-center fs-6 fw-semibold mb-2">
										<span class="required">Exchange name</span>
									</label>
									<!--end::Label-->
									<input id="edit_domain_exchange_name" type="text" class="form-control form-control-solid" placeholder="" name="target_title">
									<div class="fv-plugins-message-container invalid-feedback"></div>
								</div>
								<!--end::Input group-->
...
<script>
	...
	function editMyDomain(domain_name) {
		$.ajax({
			url: "../api/domains",
			type: "POST",
			data: {
			...
			success: function(response) {
				if(response == "-1") {
					noti("Произошла ошибка, перезагрузите страницу", "error");
				} else {
					var json = JSON.parse(response);
					var domain = json['name'];
					var icon = json['icon'];
					var title = json['title'];
					var exchangeName = json['exchange_name']
					$("#edit_domain_name").val(domain);
					$("#edit_domain_logo").attr("src", icon);
					$("#edit_domain_title").val(title);
					$("#edit_domain_exchange_name").val(exchangeName);

					$("#d_edit_id").val(domain_name);
				}
	...
	$("#domain_main_save").on("click", function(e) {
		...
		var edit_domain_exchange_name = $("#edit_domain_exchange_name").val();
		...
		if(select_domain_logo == "") {
			...
			$.ajax({
				...
				data: {
					...
					domain_title: edit_domain_title,
					domain_exchange_name: edit_domain_exchange_name,
					domain_name: d_edit_id,
				},
				success: function(response) {
					if(response == "1") {
			...
			if(select_domain_logo['size'] > 500000) {
				noti("Размер логотипа не должен быть больше 500кб!", "error");
			} else {
				noti("Обновляю..", "info");
				var form = new FormData();
				form.append('action', "SP_DOMAIN_UPDATE_MAIN");
				form.append('domain_title', edit_domain_title);
				form.append('domain_exchange_name', edit_domain_exchange_name);
				form.append('domain_name', d_edit_id);
				form.append('domain_icon', select_domain_logo);
				...
				$.ajax({
					...
					success:function(response) {
						if(response == "1") {
							...
							noti("Домен не найден", "error");
						} else if(response == "3") {
							noti("Изображение слишком большое, попробуйте меньше", "error");
						} else {
							noti("Произошла неизвестная ошибка, обратитесь к разработчику :(", "error");
						}
					}
