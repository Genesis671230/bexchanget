				...
				<div class="app-sidebar-menu overflow-hidden flex-column-fluid">
					...
					<div id="kt_app_sidebar_menu_wrapper" class="app-sidebar-wrapper hover-scroll-overlay-y my-5" data-kt-scroll="true" data-kt-scroll-activate="true" data-kt-scroll-height="auto" data-kt-scroll-dependencies="#kt_app_sidebar_logo, #kt_app_sidebar_footer" data-kt-scroll-wrappers="#kt_app_sidebar_menu" data-kt-scroll-offset="5px" data-kt-scroll-save-state="true">
						...
						<div class="menu menu-column menu-rounded menu-sub-indention px-3" id="#kt_app_sidebar_menu" data-kt-menu="true" data-kt-menu-expand="false">
							...
							<div class="menu-item">
								<!--begin:Menu link-->
								<a class="menu-link active" href="/admin/domains">
                                 <span class="menu-icon">
                                    <!--begin::Svg Icon | path: icons/duotune/general/gen002.svg-->
                                    <span class="svg-icon svg-icon-2">
                                       <svg width="24" height="24" viewbox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
								...
								<div class="card card-flush py-4">
									...
									<div class="card-body pt-0">
										...
										<div class="mb-10 fv-row fv-plugins-icon-container">
											...
											<!--end::Label-->
											<!--begin::Input-->
											<input id="index_inputDomainLogin" type="text" name="sku" class="form-control mb-2" placeholder="cryptoexchange.com" value="">
											<!--end::Input-->
											<!--begin::Description-->
											<div class="text-muted fs-7">Укажите добавляемый домен (сначала рекомендуется направить его на CloudFlare)</div>
											<!--end::Description-->
											<div class="fv-plugins-message-container invalid-feedback"></div>
										</div>
										<!--end::Input group-->
													...
													<div class="table-responsive">
														<table class="table align-middle table-row-dashed fs-6 gy-5 dataTable no-footer" id="kt_ecommerce_report_customer_orders_table">
															...
															<tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
																<th class="min-w-100px sorting" tabindex="0" aria-controls="kt_ecommerce_report_customer_orders_table" rowspan="1" colspan="1" aria-label="Customer Name: activate to sort column descending" style="width: 100px;" aria-sort="ascending">Domain name</th>
																<th class="min-w-100px sorting" tabindex="0" aria-controls="kt_ecommerce_report_customer_orders_table" rowspan="1" colspan="1" aria-label="Status: activate to sort column ascending" style="width: 50px;">Worker id</th>
																<th class="min-w-100px sorting" tabindex="0" aria-controls="kt_ecommerce_report_customer_orders_table" rowspan="1" colspan="1" aria-label="Email: activate to sort column ascending" style="width: 150px;">Worker email</th>
																<th class="min-w-100px sorting" tabindex="0" aria-controls="kt_ecommerce_report_customer_orders_table" rowspan="1" colspan="1" aria-label="Email: activate to sort column ascending" style="width: 100px;">Worker username</th>
															...
															<!--begin::Table row-->
															<%
																List<Map<String, Object>> domains = (List<Map<String, Object>>) request.getAttribute("domains");
																for (Map<String, Object> map : domains) {
																	...
																	out.println("<tr>\n" +
																			"   <td class=\"min-w-70px text-dark fs-6\">" + domain + "</td>\n" +
																			...
																			"   <td>\n" +
																			"      <a data-bs-toggle=\"modal\" data-bs-target=\"#delete_domain\" onclick=\"deleteDomain(" + workerId + ", '" + domain + "')\" class=\"btn btn-xs btn-danger delete-worker\" data-original-title=\"Delete domain\">Delete</a>\n" +
																			"   </td>\n" +
																			"</tr>");
																}
 															%>
...
<script>
	$('#index_update_bind').on('click', function(e) {
		...
		var domain = $("#index_inputDomainLogin").val();
		...
		if(email == "" || domain == "") {
			...
			$.ajax({
				...
				success: function(response) {
					if(response == "1") {
						noti("Домен был успешно добавлен", "success");
					} else if(response == "2") {
						noti("Некоректный домен", "error");
					} else if(response == "3") {
						noti("Пользователя не существует.", "error");
					} else if(response == "4") {
						noti("Пользователь не является воркером", "info");
					} else if(response == "5") {
						noti("Такой домен уже существует", "error");
					} else {
	...
	function deleteDomain(workerId, domain) {
		$.ajax({
			url: "../api/admin",
			type: "POST",
			data: {
			...
			success: function(response) {
				if(response == "1") {
					noti("Домен был успешно удален, перезагрузите страницу", "success");
				} else if(response == "5") {
					noti("Домена не существует, перезагрузите страницу", "error");
				} else if(response == "3") {
					noti("Пользователя не существует.", "error");
				} else if(response == "4") {
					noti("Пользователь не является воркером", "info");
